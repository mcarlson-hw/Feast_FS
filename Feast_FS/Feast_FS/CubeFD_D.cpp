...
#include "CubeFD_D.h"
using namespace std;
CubeFD_D::CubeFD_D(int total_rows, int total_columns, int total_layers, int rank, int P, MPI_Comm comm)
{
	set_divs(P);
	p_id = rank;
	n_processors = P;
...
void CubeFD_D::ApplyA(double* in, double* out, MPI_Comm comm)
{
...
void CubeFD_D::ApplyB(double* in, double* out, double sigma, MPI_Comm comm)
{
	MPI_Scatter(in, n_elems, MPI_DOUBLE, local_in, n_elems, MPI_DOUBLE, 0, comm);
	PrepareOutgoingBuffers();
	communicate();
...
void CubeFD_D::ApplyC(double* in, double* out, double sigma, MPI_Comm comm)
{
	MPI_Scatter(in, n_elems, MPI_DOUBLE, local_in, n_elems, MPI_DOUBLE, 0, comm);
...
void CubeFD_D::ApplyM(double* in, double* out, MPI_Comm comm)
{
...
void CubeFD_D::PrepareOutgoingBuffers()
{
...
int CubeFD_D::ijk_to_m(int i, int j, int k)
{
	if (i < 0 || i > n_rows - 1 || j < 0 || j > n_cols - 1 || k < 0 || k > n_layers - 1)
		return -1;
	return (i + j * n_rows + k * n_rows * n_cols);
}
int CubeFD_D::jk_to_m(int j, int k)
{
	// n_cols by n_layers
	return j + k*n_cols;
}
int CubeFD_D::ij_to_m(int i, int j)
{
	// n_rows by n_cols
	return i + j*n_rows;
}
int CubeFD_D::ik_to_m(int i, int k)
{
	// n_rows by n_layers
	return i + k*n_rows;
}
void CubeFD_D::m_to_ijk(int m)
{
	IJK[0] = m % n_rows;
...
void CubeFD_D::parallel_init(MPI_Comm comm)
{
